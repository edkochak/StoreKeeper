...
def validate_revenue_amount(amount_str: str) -> float:
    ...
    if not re.match(r'^[0-9]+(\.[0-9]+)?$', amount_str):
        raise ValueError(f"Неверный формат суммы: {amount_str}. Используйте только цифры и точку/запятую.")
...
def is_valid_store_name(name: str) -> bool:
    ...
    dangerous_patterns = [
        r'--', r'\/\*', r'\*\/', r';', r'DROP', r'DELETE', r'UPDATE', r'INSERT',
        r'SELECT', r'UNION', r'ALTER', r'CREATE', r'TRUNCATE'
    ]
    
    for pattern in dangerous_patterns:
        if re.search(pattern, name, re.IGNORECASE):
            logger.warning(f"Suspicious store name detected (possible SQL injection): {name}")
            return False
...
def is_valid_phone(phone: str) -> bool:
    ...
    return bool(re.match(russian_pattern, clean_phone) or re.match(generic_pattern, clean_phone))
