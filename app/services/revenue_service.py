...
class RevenueService:
    ...
    async def export_report(self) -> Tuple[bytes, Dict[str, bytes]]:
        ...
        with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
            ...
            if df.empty:
                pd.DataFrame(columns=['Магазин', 'Общая выручка', 'План', '% выполнения']).to_excel(
                    writer, sheet_name=sheet2_name, index=False
                )
            else:
                # Группируем данные по магазинам
                summary_df = df.groupby('store_name').agg({
                    'amount': 'sum', 
                    'plan': 'first'
                }).reset_index()
                
                # Добавляем колонку с процентом выполнения плана
                summary_df['percent'] = (summary_df['amount'] / summary_df['plan'] * 100).round(1)
                
                # Переименовываем колонки и записываем на лист
                summary_df.columns = ['Магазин', 'Общая выручка', 'План', '% выполнения']
    ...
    async def create_revenue(self, amount: float, store_id: int, manager_id: int, date_obj: datetime.date) -> Revenue:
        """
        ...
        revenue = Revenue(store_id=store_id, amount=amount, date=date_obj, manager_id=manager_id)
        self.session.add(revenue)
        await self.session.commit()
        await self.session.refresh(revenue)
        return revenue

    async def get_revenue(self, store_id: int, date_str: Union[str, datetime.date]) -> Optional[Revenue]:
        """
    ...
    async def _get_revenue_stats(self) -> List[Dict[str, Any]]:
        ...
        for store in stores:
            ...
            total_query = select(func.sum(Revenue.amount)).where(Revenue.store_id == store.id)
            ...
            stats.append({
                ...
                "last_revenue": {
                    ...
                    "date": last_revenue.date.isoformat() if last_revenue and isinstance(last_revenue.date, datetime.date) else 
                            last_revenue.date if last_revenue else None
                }
            })
