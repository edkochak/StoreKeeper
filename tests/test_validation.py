...
def test_revenue_amount_validation():
    ...
    with pytest.raises(ValueError):
        validate_revenue_amount("-100")  # отрицательные значения
    with pytest.raises(ValueError):
...
def test_date_format_validation():
    ...
    with pytest.raises(ValueError):
        validate_date_format("32.05.2023")  # несуществующая дата
    with pytest.raises(ValueError):
        validate_date_format("2023.05.01")  # неправильный порядок
    with pytest.raises(ValueError):
        validate_date_format("вчера")  # текстовая дата
...
def test_store_name_validation():
    ...
    assert not is_valid_store_name("Магазин;DROP TABLE stores;")  # SQL инъекция
