...
async def test_get_month_total(session):
    ...
    manager = await user_svc.get_or_create("Month", "Total", "manager", store_id=store.id)
    ...
    await rev_svc.create_revenue(500.0, store.id, manager.id, date(current_year, current_month, 10))
    await rev_svc.create_revenue(500.0, store.id, manager.id, date(current_year, current_month, 20))
    
    # Прошлый месяц, запись на 300
    prev_month = current_month - 1 if current_month > 1 else 12
    prev_year = current_year if current_month > 1 else current_year - 1
    await rev_svc.create_revenue(300.0, store.id, manager.id, date(prev_year, prev_month, 15))
